...
function EditLearningPath() {
  ...
  useEffect(() => {
    const fetchLearningPath = async () => {
      try {
        ...
        // Verify that the current user is the creator of the path
        if (path.userId !== currentUser?._id && path.userId !== currentUser?.id) {
          setError('You do not have permission to edit this learning path');
        ...
        setRequirements(path.requirements || '');
        setDifficulty(path.difficulty || 'Beginner');
        setDuration(path.duration || 30);
        setTags(path.tags || []);
        setTips(path.tips || '');
        setIsPublic(path.isPublic !== undefined ? path.isPublic : true);
        setUserId(path.userId);
        
        // Set milestones, making sure each has a resources array
        if (path.milestones && path.milestones.length > 0) {
          setMilestones(path.milestones.map(m => ({
            ...m,
            resources: m.resources || []
          })));
        } else {
          // Default milestone if none exist
          setMilestones([{ 
            title: '', 
            description: '', 
            orderIndex: 0,
        ...
        setError('Failed to fetch learning path details. Please try again.');
        setFetchLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Use the user ID for authorization
      const currentUserId = currentUser?._id || currentUser?.id;
      
      await LearningPathService.updatePath(pathId, learningPathData, currentUserId);
      navigate(`/learning-paths/${pathId}`);
      
    } catch (err) {
      console.error('Failed to update learning path:', err);
      if (err.response?.status === 403) {
        setError('You do not have permission to update this learning path');
      } else {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        >
          <FaArrowLeft className="mr-2" /> Back to Learning Path
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 md:p-8">
        <div className="flex items-center justify-center mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-800 ml-4">Edit Learning Path</h1>
        </div>
